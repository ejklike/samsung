...
def tower_loss(scope, signals, labels):
  ...
  for l in losses + [total_loss]:
    ...
    loss_name = re.sub('%s_[0-9]*/' % my_graph.TOWER_NAME, '', l.op.name)
...
def train():
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    lr = tf.train.exponential_decay(my_graph.INITIAL_LEARNING_RATE,
                                    global_step,
                                    my_graph.DECAY_STEPS,
                                    my_graph.LEARNING_RATE_DECAY_FACTOR,
                                    staircase=True)
    ...
    with tf.variable_scope(tf.get_variable_scope()):
      for i in range(FLAGS.num_gpus):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (my_graph.TOWER_NAME, i)) as scope:
            # Dequeues one batch for the GPU
            signal_batch, label_batch = batch_queue.dequeue()
            # Calculate the loss for one tower of the CIFAR model. This function
            # constructs the entire CIFAR model but shares the variables across
            # all towers.
            loss = tower_loss(scope, signal_batch, label_batch)
    ...
    variable_averages = tf.train.ExponentialMovingAverage(
        my_graph.MOVING_AVERAGE_DECAY, global_step)
