...
def train():
  ...
  with tf.Graph().as_default():
    ...
    with tf.device('/cpu:0'):
      ...
      signals_trn, labels_trn= my_graph.inputs_har(
        FLAGS.data_fname, train=True, val=False)
      signals_tst, labels_tst = my_graph.inputs_har(
        FLAGS.data_fname, train=False)
    ...
    with tf.train.MonitoredTrainingSession(
      ...
      while not mon_sess.should_stop():
        ...
        print(mon_sess.graph.get_tensor_by_name('inference/conv1:weights'))
...
if __name__ == '__main__':
  ...
  # Input filename
  if args.device_id > 0:
    FLAGS.data_fname = './PM{}.p'.format(args.device_id)
    FLAGS.record_fname = './PM{}.csv'.format(args.device_id)
  else:
    FLAGS.data_fname = './Drill.p'.format(args.device_id)
    FLAGS.record_fname = './Drill.csv'.format(args.device_id)
